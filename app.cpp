  ...
  irs::gpio_pin_t* ap_cs_pin)
{
  ...
  // Присвоения, чтобы не выдавались предупреждения компиллятора
  ap_spi = ap_spi;
  ap_cs_pin = ap_cs_pin;
...
void u309m::app_t::tick()
{
  ...
  if (m_eeprom.error()) {
    ...
    irs::mlog() << " Ошибка CRC в tick app" << endl;
    ethernet_to_eeprom();
  }
  ...
  if (m_alarm_timer.check())
  {
    ...
    if (!m_refresh_timeout)
    {
      ...
      {
        //irs::mlog() << "Обновление уставок всех источников начато" << endl;
    ...
    else
    {
      if (m_refresh_timer.check())
      {
        ...
        m_refresh_timeout = false;
        //irs::mlog() << "Обновление уставок всех источников закончено" << endl;
      }
    }
